...
Add Varn from the [App Store listing](https://apps.shopify.com/varn-variants-swatches). Shopify shows you the permissions before you approve them. See [Permissions and privacy](reference/permissions-privacy) for what each one is used for and why.

After approving, you land on the Varn dashboard on Starter, which has no time limit. Every paid plan is [free for 7 days](billing/trial).
