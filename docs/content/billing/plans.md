...
Three things are true of every paid feature, and they are worth knowing before you upgrade:

1. **Locked features look locked.** They render visibly disabled with the plan named. You will never press a control that then reveals a paywall.
...
Every change goes through Shopify's own confirmation page, so you always see what you are agreeing to before anything is charged. See [Manage your plan](billing/manage).

## The full feature grid

For a line-by-line comparison, see [Feature matrix](reference/feature-matrix).
